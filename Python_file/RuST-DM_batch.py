...
# User input data GIS files
dem_dataset = r"E:\RuST\RuST-DM\Validation_data\case_data\dem\dem_point"
household_dataset = r"E:\RuST\RuST-DM\Validation_data\case_data\house\xx"
road_dataset = r"E:\RuST\RuST-DM\Validation_data\case_data\road\road_line"
output_folder = r"E:\RuST\RuST-DM\Validation_data\case_data\XX"
# **********************************************************************************************************************
...
all_files = [i[2] for i in os.walk(dem_dataset)][0]
name_list = [i for i in all_files if i[-4:] == ".shp"]

for name in name_list[-9:-8]:
    # set output folder
    sub_output = output_folder + "\\" + name[:-4]
    check_folder(sub_output)
    EM_folder = sub_output + "\\" + 'EM_results'
    MM_folder = sub_output + "\\" + 'MM_results'
    check_folder(EM_folder)
    check_folder(MM_folder)

    # set input data (shpefile)
    household_file = household_dataset + "\\" + name
    road_file = road_dataset + "\\" + name
    DEM_ENVI_file = dem_dataset + "\\" + name
    ...
    results = tidy_results(wtp_dict, source_dict, nodes_dict, road_dict, road_pair, near_road, standard, technology,
                           price,
    ...
    EM_dict = {"Total Household": len(household_nodes), "Treated Household": household_num, "Coverage": rate,
               "Degree": DD_EM,
               "Total Cost": total, "WTP Capex": capex_w, "WTP Opex": opex_w, "Sewer Capex": capex_s,
               "Sewer Opex": opex_s}
    print("\n**************************************************************************************")
    ...
    EM_results = sub_output + "\\" + "EM_results.csv"
    ...
    if len(wtp_dict.keys()) > 1:
        MM_r = merging_module(wtp_dict, sewer_dict, source_dict, nodes_dict, dem, near_road, road_dict, road_pair,
                              standard,
        ...
        new_MM_r = tidy_results(WTPs_dict, sources_dict, nodes_dict, road_dict, road_pair, near_road, standard,
                                technology,
                                price, pipe_type, Q, POP, coeff, depth_min, depth_max, wide, slope, f_ls, s_ls)
    ...
    total = round(capex_w + capex_s + opex_w + opex_s, 2)
    MM_dict = {"Total Household": len(household_nodes), "Treated Household": household_num, "Coverage": rate,
               "Degree": DD_EM,
               "Total Cost": total, "WTP Capex": capex_w, "WTP Opex": opex_w, "Sewer Capex": capex_s,
               "Sewer Opex": opex_s}
    print("\n**************************************************************************************")
    ...
    MM_results = sub_output + "\\" + "MM_results.csv"
...
print("***************************************************")
print("%d cases in total" % len(name_list))
print(time.asctime(time.localtime(time.time())))
print("***************************************************")
